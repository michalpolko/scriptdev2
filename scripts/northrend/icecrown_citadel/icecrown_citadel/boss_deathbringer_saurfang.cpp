...
{
    {-461.512f, 2211.11f, 541.197f, 3.14159f},  //  0 Deathbringer spawn point (behind door)
    {-491.30f, 2211.35f, 541.11f, 3.16f},       //  1 Deathbringer dest point (in front of door)
    {-559.82f, 2211.29f, 539.30f, 6.28f},       //  2 main npc summon/despawn point
    {-561.87f, 2215.94f, 539.30f, 6.28f},       //  3 guard npc1 summon/despawn point
    {-560.17f, 2214.17f, 539.30f, 6.28f},       //  4 guard npc2 summon/despawn point
    {-560.17f, 2207.71f, 539.30f, 6.28f},       //  5 guard npc3 summon/despawn point
    {-561.87f, 2205.68f, 539.30f, 6.28f},       //  6 guard npc4 summon/despawn point
    {-534.82f, 2211.29f, 539.30f, 6.28f},       //  7 main npc first move
    {-536.87f, 2215.94f, 539.30f, 6.28f},       //  8 guard npc1 first move
    {-535.17f, 2214.17f, 539.30f, 6.28f},       //  9 guard npc2 first move
    {-535.17f, 2207.71f, 539.30f, 6.28f},       // 10 guard npc3 first move
    {-536.87f, 2205.68f, 539.30f, 6.28f}        // 11 guard npc4 first move
};
 ...
 * passive guards AI
 */
class MANGOS_DLL_DECL npc_deathbringer_event_guards_iccAI : public ScriptedAI
{
    public:
...
/**
 * Highlord Saurfang - event handler
 */
class MANGOS_DLL_DECL npc_highlord_saurfang_deathbringer_event_iccAI : public base_icc_creatureAI
{
    private:
    ...
    public:
        npc_highlord_saurfang_deathbringer_event_iccAI(Creature* pCreature) : base_icc_creatureAI(pCreature)
        {
        ...
        void DoStartDeathbringerEvent()
        {
            m_bIsEventStarted = true;
            NextStep(0);
        }

        void DoFinishDeathbringerEvent()
        {
            NextStep();
        }
...
CreatureAI* GetAI_npc_highlord_saurfang_deathbringer_event_icc(Creature* pCreature)
{
    return new npc_highlord_saurfang_deathbringer_event_iccAI(pCreature);
}

class MANGOS_DLL_DECL boss_deathbringer_saurfangAI : public base_icc_creatureAI
{
    private:
    ...
    public:
        boss_deathbringer_saurfangAI(Creature* pCreature) : base_icc_creatureAI(pCreature)
        {
    ...
    void DoSummonEventNpc()
    {
        ...
        if (Creature *pTmp = m_creature->SummonCreature(m_bIsAlliance ? NPC_MURADIN_BRONZEBEARD : NPC_OVERLORD_SAURFANG, fPositions[2][0], fPositions[2][1], fPositions[2][2], fPositions[2][3], TEMPSUMMON_MANUAL_DESPAWN, 0))
        {
            ...
            if (npc_highlord_saurfang_deathbringer_event_iccAI *pTmpAI = dynamic_cast<npc_highlord_saurfang_deathbringer_event_iccAI*>(pTmp->AI()))
                pTmpAI->DoStartDeathbringerEvent();
        }
    }
    ...
    void Aggro(Unit *pWho)
    {
        ...
        DoCastSpellIfCan(m_creature, SPELL_BLOOD_POWER, CAST_TRIGGERED); // 72370 in creature_template_addon but this one also increases damage done
    }

    // custom evading - return to point in front of the door
    void EnterEvadeMode()
    {
        ...
        if (m_creature->isAlive())
            m_creature->GetMotionMaster()->MovePoint(10, fPositions[1][0], fPositions[1][1], fPositions[1][2], false);
    ...
    void MovementInform(uint32 uiMovementType, uint32 uiData)
    {
        ...
        if (uiData == 10)
        {
            m_creature->SetOrientation(3.14f);
            JustReachedHome();
        }
    }
    ...
    void JustDied(Unit *pKiller)
    {
        ...
        if (Creature *pTmp = m_creature->GetMap()->GetCreature(m_guidEventNpcGuid))
        {
            ...
            {
                if (npc_highlord_saurfang_deathbringer_event_iccAI *pTmpAI = dynamic_cast<npc_highlord_saurfang_deathbringer_event_iccAI*>(pTmp->AI()))
                    pTmpAI->DoFinishDeathbringerEvent();
            }
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_bIsHeroic)
        {
            if (m_uiScentOfBloodTimer <= uiDiff)
            {
                ...
                // maybe add this as triggered in core?
...
void AddSC_boss_deathbringer_saurfang()
{
    ...
    pNewscript = new Script;
    pNewscript->Name = "npc_highlord_saurfang_deathbringer_event_icc";
    pNewscript->GetAI = &GetAI_npc_highlord_saurfang_deathbringer_event_icc;
    pNewscript->RegisterSelf();
