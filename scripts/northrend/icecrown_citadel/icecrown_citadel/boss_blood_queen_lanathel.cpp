...
static const float QueenPosition[2][4] =
{
    {4595.640137f, 2769.195557f, 400.137054f},  // 0 center, ground
    {4595.904785f, 2769.315918f, 421.838623f},  // 1 center, air
};
...
struct MANGOS_DLL_DECL boss_blood_queen_lanathelAI : public ScriptedAI
{
        ...
        if (uiData == POINT_CENTER_GROUND)
        {
            if (m_uiPhase == PHASE_RUNNING)
            {
                ...
                if (DoCastSpellIfCan(m_creature, SPELL_INCITE_HORROR) == CAST_OK)
                {
                    ...
                    m_creature->GetMotionMaster()->MovePoint(POINT_CENTER_AIR, QueenPosition[1][0], QueenPosition[1][1], QueenPosition[1][2], false);
                }
            }
            else
            {
    ...
    void SetLevitate(bool bLevitateState)
    {
        m_creature->SetLevitate(bLevitateState);
    ...
    Unit* SelectClosestFriendlyTarget(Unit *pVictim)
    {
        ...
        if (m_pInstance)
        {
            ...
            {
                // don't target current victim
                if ((*itr).getSource()->GetObjectGuid() == pVictim->GetObjectGuid())
                    continue;
    ...
    Unit* SelectRandomRangedTarget(Unit *pSource)
    {
        ...
        if (!lTargets.empty())
        {
            uint8 max = m_pInstance->Is25ManDifficulty() ? 8 : 3;
        ...
        switch (m_uiPhase)
        {
            case PHASE_GROUND:
            {
                ...
                if (m_uiPhaseTimer <= uiDiff)
                {
                    ...
                    m_creature->GetMotionMaster()->MovePoint(POINT_CENTER_GROUND, QueenPosition[0][0], QueenPosition[0][1], QueenPosition[0][2]);
                ...
                {
                    // not sure who should cast on whom
                ...
                if (m_uiSwarmingShadowsTimer <= uiDiff)
                {
                     ...
                     * FIXME: spell summoning NPCs has problem with error: "You are in the wrong zone"
                     */
...
struct MANGOS_DLL_DECL mob_swarming_shadowsAI : public ScriptedAI
{
    ...
    {
        m_creature->SetDisplayId(11686); // FIXME: update in DB
        SetCombatMovement(false);
